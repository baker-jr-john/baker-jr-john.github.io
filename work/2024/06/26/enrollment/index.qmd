...
# Calculate progression rates for other grades
...
for (grade in names(smoothed_values)) {
  last_value <- tail(smoothed_values[[grade]], 1)
  projections[[grade]] <- c(last_value, rep(NA, length(years) - 1))
...
for (year in years) {
  projections[projections$Year == year, "Prekindergarten"] <- population[population$Year == year, "3- to 5-year-olds"] * enrollment_rates$Prekindergarten_rate
  projections[projections$Year == year, "Kindergarten"] <- population[population$Year == year, "5-year-olds"] * enrollment_rates$Kindergarten_rate
  projections[projections$Year == year, "1st_grade"] <- population[population$Year == year, "6-year-olds"] * enrollment_rates$first_grade_rate
  projections[projections$Year == year, "Ungraded_PK_8"] <- population[population$Year == year, "5- to 13-year-olds"] * enrollment_rates$Ungraded_PK_8_rate
  projections[projections$Year == year, "Ungraded_9_12"] <- population[population$Year == year, "14- to 17-year-olds"] * enrollment_rates$Ungraded_9_12_rate
}
...
There are numerous tools to select from when tackling an exercise like this. Instead of delving into the various options, I decided to simply tell you what I used:
...
To use the data, I initially had to modify the spreadsheet in Excel. First, I un-merged cells A3:A4, B3:B4, and D4:E4. Then, I moved "Year" and "All" from cells A3 and B3 to cells A4 and B4. I also had to modify the ramining cells in row 4:
...
Additionally, I had to delete "\\4" from cell A37. Finally, I deleted row 5. I converted the spreadsheet from the *`.xls`* file format to *`.xlsx`* by saving it as *`.xlsx`*. After the data was in a usable state, I moved the project to RStudio.
...
# Rename the columns for convenience
colnames(enrollment) <- c("Year", "Prekindergarten", "Kindergarten", "1st_grade", "2nd_grade", "3rd_grade",
                          "4th_grade", "5th_grade", "6th_grade", "7th_grade", "8th_grade", "9th_grade", 
                          "10th_grade", "11th_grade", "12th_grade", "Ungraded_PK_8", "Ungraded_9_12")
...
# Calculate progression rates for other grades
progression_rates <- enrollment %>%
  summarise(across(starts_with("grade"), list(rate = ~mean(.x / lag(.x, 1), na.rm = TRUE)), .names = "{col}_rate"))
...
In this section of code, the `gt` library is loaded to facilitate the creation of well-formatted tables. The code then constructs a data frame, `forecast_table_df`, which contains projected enrollment data for various grade levels. This data frame is derived from the `filtered_data` object, specifically filtering for projected enrollment data (`Type == "Projected"`). It selects the relevant columns: `Year`, `Prekindergarten`, `Kindergarten`, and grades from `1st_grade` to `12th_grade`. These grade columns are then renamed to more readable names, such as "First Grade" for `1st_grade`, "Second Grade" for `2nd_grade`, and so on. The renamed data frame is then passed to the `gt` function to create a table object, `forecast_table`. A header is added to this table with the title "Enrollment in public elementary and secondary schools, by grade (in thousands)". Finally, the table is displayed. This process results in a well-organized and readable table that presents the projected enrollment figures for each grade level.
...
forecast_table_df <- filtered_data %>%
  ...
  rename(
    ...
    "Second Grade" = `2nd_grade`,
    "Third Grade" = `3rd_grade`,
    "Fourth Grade" = `4th_grade`,
    "Fifth Grade" = `5th_grade`,
    "Sixth Grade" = `6th_grade`,
    "Seventh Grade" = `7th_grade`,
    "Eighth Grade" = `8th_grade`,
    "Ninth Grade" = `9th_grade`,
    "Tenth Grade" = `10th_grade`,
    "Eleventh Grade" = `11th_grade`,
    "Twelfth Grade" = `12th_grade`
  )
...
forecast_table <- forecast_table %>%
  tab_header(title = "Enrollment in public elementary and secondary schools, by grade (in thousands)")
...
# Create a data frame for elementary grades
elementary_table_df <- filtered_data %>%
  filter(Type == "Projected") %>%
  select(Year, Prekindergarten, Kindergarten, `1st_grade`, `2nd_grade`, `3rd_grade`, `4th_grade`, `5th_grade`) %>%
  rename(
    `First Grade` = `1st_grade`,
    `Second Grade` = `2nd_grade`,
    `Third Grade` = `3rd_grade`,
...
# Create the gt table for elementary grades
elementary_table <- gt(elementary_table_df)

# Add a table header to the elementary table
elementary_table <- elementary_table %>%
  tab_header(title = "Enrollment in public elementary schools, by grade (Prekindergarten to Fifth Grade, in thousands)")

# Display the elementary table
elementary_table
```
...
# Create a data frame for middle and high school grades
middle_high_table_df <- filtered_data %>%
  filter(Type == "Projected") %>%
  select(Year, `6th_grade`, `7th_grade`, `8th_grade`, `9th_grade`, `10th_grade`, `11th_grade`, `12th_grade`) %>%
  rename(
    `Sixth Grade` = `6th_grade`,
    `Seventh Grade` = `7th_grade`,
    `Eighth Grade` = `8th_grade`,
...
# Create the gt table for middle and high school grades
middle_high_table <- gt(middle_high_table_df)

# Add a table header to the middle and high school table
middle_high_table <- middle_high_table %>%
  tab_header(title = "Enrollment in public secondary schools, by grade (Sixth Grade to Twelfth Grade, in thousands)")

# Display the middle and high school table
middle_high_table
```

### Data Combination and Visualization

The code performs a series of data manipulation and visualization tasks to create a line plot that displays the projected enrollment in public elementary and secondary schools from 2010 to 2030. First, it combines actual and projected enrollment data into a single dataset, marking each entry as either "Actual" or "Projected." It then filters this combined dataset to include only the years 2010 to 2030 and arranges the data in chronological order. New columns are created to represent the total enrollment for PK-Grade 8, Grades 9-12, and the overall Total enrollment. The data is then reshaped into a long format suitable for plotting with `ggplot2`.

In the visualization step, a line plot is created using `ggplot2`, with different colors representing the categories Total, PK-Grade 8, and Grades 9-12. The x-axis shows the years, and the y-axis shows the enrollment in thousands. Vertical dashed lines and annotations indicate the separation between actual and projected data. The plot is customized with specific colors, labels, and a minimal theme. Finally, the plot is printed, displaying the enrollment trends over time with clear distinctions between different grade levels.
...
combined_data <- bind_rows(
  enrollment %>% mutate(Type = "Actual"),
  projections %>% mutate(Type = "Projected")
)

# Filter data for the years 2010 to 2030
filtered_data <- combined_data %>% filter(Year >= 2010 & Year <= 2030)
...
plot_data <- filtered_data %>%
  select(Year, PK_Grade_8, Grades_9_12, Total) %>%
  pivot_longer(cols = c(PK_Grade_8, Grades_9_12, Total), names_to = "Category", values_to = "Enrollment") %>%
  mutate(Category = factor(Category, levels = c("Total", "PK_Grade_8", "Grades_9_12")))
...
enrollment_plot <- ggplot(plot_data, aes(x = Year, y = Enrollment / 1000, color = Category)) +
  ...
  annotate("text", x = 2015, y = max(plot_data$Enrollment) / 1000 * 0.9, label = "Actual", size = 4) +
  annotate("text", x = 2025, y = max(plot_data$Enrollment) / 1000 * 0.9, label = "Projected", size = 4) +
  scale_color_manual(
    values = c("Total" = "#009E73", "PK_Grade_8" = "#E69F00", "Grades_9_12" = "#56B4E9"),
    labels = c("Total", "PK-Grade 8", "Grades 9-12")
  ) +
  scale_y_continuous(labels = scales::comma) +

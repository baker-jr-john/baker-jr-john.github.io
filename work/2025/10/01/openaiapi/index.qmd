...
If you accidentally share your code or upload it to a public repository like GitHub, your secret key will be exposed, allowing anyone to use your account and incur charges on your behalf. Unlike Dr. Li's Google Colab approach, where the key can be stored in Colab's secure environment, we need a different strategy when working locally.
...
Fortunately, there are several ways we can manage this securely. Dr. Li discussed one approach using Google Colab's built-in secrets management. For local development, we'll use a tool called `dotenv`, which helps you manage application secrets and configuration by loading them from a special file into your program's environment variables.
...
3. Click **"Create new secret key"** and create one if you haven't already, as Dr. Li previously discussed.
4. Copy the key immediately and save it somewhere safe. You won't be able to see it again after you close the window.
...
Open your `main.py` file and write the Python code. It will first load the variables from the `.env` file and then use the key to make an API call. Here's the code I wrote with detailed explanations:
...
def get_chatbot_response(user_prompt, model="gpt-4o-mini"):
    ...
    This approach differs from Dr. Li's linear script by encapsulating
    the API logic in a reusable function that can be called multiple times
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "You provide insightful answers about implementing large language models in "
                        "educational contexts and explain technical concepts clearly with practical, "
                        "real-world examples."
                    ),
                },
                {
...
if __name__ == "__main__":
    # Define your prompt
    # Unlike Dr. Li's hardcoded approach, you can easily change this
    # or even make it interactive by using input() to ask the user for questions
    prompt = (
...
## Key Differences from Dr. Li's Approach

This implementation differs from Dr. Li's Google Colab version in several important ways. Dr. Li used a linear script that runs from top to bottom to perform a single, specific task with a hardcoded prompt. To ask a different question, you would need to manually edit the content of the user's message within the code.

My script is more modular because it defines a reusable function, `get_chatbot_response(user_prompt, model)`. This function encapsulates all the logic for making the API call, which means you can easily call this function multiple times with different prompts without rewriting code. The use of `if __name__ == "__main__":` is a standard Python convention that allows the script to be run directly or imported as a module into a larger application.

Additionally, Dr. Li's code lacks comprehensive error handling. If the API key is missing or the network call fails, the script will crash with an unhandled error. My script includes robust error handling that first checks if it found the API key and returns a helpful message if not. It also wraps the API call in a `try...except` block to gracefully catch any other potential issues, like network problems or API errors, and report them without crashing.

Finally, there's a strategic difference in model selection. While Dr. Li uses `gpt-3.5-turbo`, I've chosen `gpt-4o-mini` as the default model for several vital reasons. `gpt-4o-mini` is more cost-effective than `gpt-3.5-turbo`, which is crucial for educational projects where budget constraints are a concern. You want to maximize experimentation within limited resources. Beyond cost savings, `gpt-4o-mini` was specifically designed as the successor to `gpt-3.5-turbo`, offering comparable or better performance across reasoning, instruction-following, and code-generation tasks while being more efficient. By using this 2024 model, you're leveraging the latest technology available, which demonstrates good practice in educational technology where staying current with tools directly benefits student learning experiences.
